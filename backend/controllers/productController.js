...
const { fetchLiveAmazonPrice, searchLiveAmazonProducts, fetchLiveAmazonProductDetails } = require('../services/amazonApiService');
...
const getProductById = asyncHandler(async (req, res) => {
  ...
  // Intercept synthetic amazon IDs generated by the search bar
  if (idStr.startsWith('amazon-')) {
     const asin = idStr.split('amazon-')[1];
     const liveDetails = await fetchLiveAmazonProductDetails(asin);
     if (liveDetails) {
        return res.json(liveDetails);
     } else {
        res.status(404);
